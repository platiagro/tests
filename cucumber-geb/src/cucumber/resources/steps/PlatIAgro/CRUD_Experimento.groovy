...
Quando(~/selecionar o operador novamente/) { ->
  ...
  Thread.sleep(3000)
  waitFor(40) { $(By.xpath("//*[@id='root']/section/section/section/aside/div/div/ul/li[2]/div")).click() } 
...
E(~/a tela de visualização de dados vai ser aberta/) { ->
  at PageProj
...
Então(~/que o usuário irá voltar ao fluxo/) { ->
  at PageProj
...
E(~/selecionar a tarefa Regressão Logística/) { ->
  ...
  sr1 = browser.driver.findElementByXPath("//*[@id='PREDICTOR\$Menu']/li[8]/div/div[2]/div/span") 
     browser.driver.executeScript("arguments[0].scrollIntoView();" , sr1)
...
E(~/o campo nome do experimento deve ser limpo/) { -> assert true }
...
Quando(~/selecionar a Tarefa presente no fluxo de experimento/) { ->
  ...
  waitFor(30) { page.selectTask.click() }
